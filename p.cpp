					...
					if(strcmp(password,"pass")==0)
					{
						...
						while(choice1!=-1)
						{
							...
							switch(choice1)
							{
								...
								case 7:
										flag=0;
										fp1.open("bookings.txt",ios::in| ios::binary);
								       	file10.open("passengers.txt",ios::in|ios::binary);
								       	cout<<"\n\tENTER THE FLIGHT NUMBER: ";
								       	cin>>flight_no;
								       	cout<<"\n\tENTER DATE: ";
								       	cin>>date;
								       	while(!fp1.eof())
								       	{
											...
											fp1.read((char *)&b5, sizeof(b5));
											if(fp)
											{
										       	if(b5.flight_no==flight_no && strcmp(b5.date,date)==0 && b5.pass_cancel==1)
												{
													
										       		while(!file10.eof())
										       		{
										       			
										       			file10.read((char *)&p5, sizeof(p5));
										       			if(file10)
														{
											       			if(strcmp(p5.dispemail(),b5.passenger_email)==0&&strcmp(p5.disp(),b5.passenger_name)==0)
											       			{
